...
void matMulTiledCornerTuningKernel(float* A, float* B, float* C){
    ...
    for(int k=0;k<ceil(1.0*M/TILE_SIZE);++k){
        ...
        if(j<K && (k*TILE_SIZE+taili)<M) Bs[tailj][taili] = B[j*M + k*TILE_SIZE + taili];
...
void matMulTiledCornerTuning(float* A, float* B, float* C){
    float *A_d, *B_d, *C_d;
    ...
    matMulTiledCornerTuningKernel<<<dimGrid, dimBlock>>>(A_d, B_d, C_d);
